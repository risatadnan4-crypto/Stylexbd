...
function saveDB() {
  try {
    ...
    // Also mirror to secondary paths to ensure cross-run persistence across hot restarts
    try {
      const secondaryPath = path.join(process.cwd(), "data", "luxury_db.json");
      if (secondaryPath !== DB_FILE) {
        writeDatabaseToPath(secondaryPath, serialized);
      }
    } catch (secErr) {}
    try {
      const tmpPath = "/tmp/luxury_db.json";
      if (tmpPath !== DB_FILE) {
        writeDatabaseToPath(tmpPath, serialized);
      }
    } catch (tmpErr) {}
...
function buildProductObject(p: any = {}, localProduct: any = {}, pm: any = {}): Product {
  ...
  const id = String(p?.id || local.id || pm?.id || `prod_${Date.now()}`);
  ...
  // Colors & dimensions JSON metadata parsing
  let pDimObj: any = {};
  if (typeof p?.dimensions === "string" && p.dimensions.trim().startsWith("{")) {
    pDimObj = tryJsonParse(p.dimensions) || {};
  }
  let localDimObj: any = {};
  if (typeof local.dimensions === "string" && local.dimensions.trim().startsWith("{")) {
    localDimObj = tryJsonParse(local.dimensions) || {};
  }
  const dimObj = { ...localDimObj, ...pDimObj };

  // Sizes - prioritizing Supabase database first
  let parsedSizes: string[] = [];
  const candidateSizes = [p?.sizes, local?.sizes, paymentMeta?.sizes, dimObj?.sizes];
  ...
  // Images - prioritize Supabase images first
  let parsedImages: string[] = [];
  const candidateImages = [p?.images, local?.images, paymentMeta?.images, dimObj?.images];
  for (const rawImgs of candidateImages) {
    ...
    if (Array.isArray(rawImgs)) {
      ...
      } else if (rawImgs.trim().startsWith("http") || rawImgs.trim().startsWith("/") || rawImgs.trim().startsWith("data:")) {
        list = [rawImgs.trim()];
      }
    }
    if (list.length > 0) {
  ...
  let rawColors = (p?.colors !== undefined && p?.colors !== null && (!Array.isArray(p.colors) || p.colors.length > 0))
    ? p.colors 
    : ((local.colors !== undefined && local.colors !== null) ? local.colors : (dimObj?.colors || []));
  ...
  // Single price field: price
  const resolvedPrice = getNum(p?.price, local.price, dimObj?.price, 0) ?? 0;
  ...
  const finalOfferPrice = (resolvedOfferPrice !== null && resolvedOfferPrice > 0 && resolvedOfferPrice < resolvedPrice)
    ? resolvedOfferPrice
    : null;

  const resolvedSeoKeywords = getStr(p?.seoKeywords, p?.seo_keywords, p?.metaKeywords, p?.meta_keywords, seoMeta.seoKeywords, dimObj?.seoKeywords, local.seoKeywords, local.seo_keywords, local.metaKeywords, "");
  const resolvedSeoTitle = getStr(p?.seoTitle, p?.seo_title, seoMeta.seoTitle, dimObj?.seoTitle, local.seoTitle, "");
  const resolvedSeoDesc = getStr(p?.seoDescription, p?.seo_description, seoMeta.seoDescription, dimObj?.seoDescription, local.seoDescription, "");
  const resolvedSeoSlug = getStr(p?.seoSlug, p?.seo_slug, seoMeta.seoSlug, dimObj?.seoSlug, local.seoSlug, "");
  const resolvedCanonical = getStr(p?.canonicalUrl, p?.canonical_url, seoMeta.canonicalUrl, dimObj?.canonicalUrl, local.canonicalUrl, "");
  const resolvedOgTitle = getStr(p?.ogTitle, p?.og_title, seoMeta.ogTitle, dimObj?.ogTitle, local.ogTitle, "");
  const resolvedOgDesc = getStr(p?.ogDescription, p?.og_description, seoMeta.ogDescription, dimObj?.ogDescription, local.ogDescription, "");
  const resolvedOgImage = getStr(p?.ogImage, p?.og_image, seoMeta.ogImage, dimObj?.ogImage, local.ogImage, "");
  const resolvedRobots = getStr(p?.robots, seoMeta.robots, dimObj?.robots, local.robots, "index, follow");

  const resolvedImageUrl = (() => {
    const direct = getStr(p?.imageUrl, p?.image_url, p?.image, local.imageUrl, dimObj?.imageUrl, dimObj?.image, "");
  ...
  // Timer must use timer_enabled and timer_end_at
  const resolvedTimerEndTime = getStr(
    p?.timer_end_at,
    p?.timer_end_time,
    p?.timerEndTime,
    p?.timerEndDate,
    paymentMeta?.timerEndTime,
    paymentMeta?.timer_end_at,
    dimObj?.timerEndTime,
    local.timerEndTime,
    local.timer_end_at,
    ""
  ...
  const resolvedTimerStartTime = getStr(
    p?.timer_start_time,
    p?.timerStartTime,
    p?.timerStartDate,
    paymentMeta?.timerStartTime,
    dimObj?.timerStartTime,
    local.timerStartTime,
    ""
  ) || null;

  const resolvedTimerMessage = getStr(
    p?.timer_message,
    p?.timerMessage,
    paymentMeta?.timerMessage,
    dimObj?.timerMessage,
    local.timerMessage,
    ""
  ) || null;

  const resolvedTimerEnabled = p?.timer_enabled !== undefined
    ? (p.timer_enabled !== false && String(p.timer_enabled) !== "false")
    : (p?.timer_active !== undefined
        ? (p.timer_active !== false && String(p.timer_active) !== "false")
        : (p?.timerEnabled !== undefined
            ? (p.timerEnabled !== false && String(p.timerEnabled) !== "false")
            : (p?.timerActive !== undefined
                ? (p.timerActive !== false && String(p.timerActive) !== "false")
                : (local.timerEnabled !== undefined
                    ? (local.timerEnabled !== false && String(local.timerEnabled) !== "false")
                    : (local.timerActive !== undefined
                        ? (local.timerActive !== false && String(local.timerActive) !== "false")
                        : !!resolvedTimerEndTime)))));

  const resolvedTimerActive = resolvedTimerEnabled;

  const resolvedDeliveryPrice = getNum(
    p?.deliveryPrice, p?.delivery_price,
    paymentMeta?.deliveryPrice,
    dimObj?.deliveryPrice,
    local.deliveryPrice,
    local.deliveryCharge,
    p?.deliveryCharge, p?.delivery_charge,
    paymentMeta?.deliveryCharge,
    dimObj?.deliveryCharge,
    100
  ) ?? 100;

  const resolvedDeliveryDhaka = getNum(p?.deliveryPriceDhaka, p?.delivery_price_dhaka, paymentMeta?.deliveryPriceDhaka, dimObj?.deliveryPriceDhaka, local.deliveryPriceDhaka, resolvedDeliveryPrice) ?? 100;
  const resolvedDeliveryChattogram = getNum(p?.deliveryPriceChattogram, p?.delivery_price_chattogram, paymentMeta?.deliveryPriceChattogram, dimObj?.deliveryPriceChattogram, local.deliveryPriceChattogram, 150) ?? 150;
  const resolvedDeliveryRajshahi = getNum(p?.deliveryPriceRajshahi, p?.delivery_price_rajshahi, paymentMeta?.deliveryPriceRajshahi, dimObj?.deliveryPriceRajshahi, local.deliveryPriceRajshahi, 150) ?? 150;
  const resolvedDeliveryKhulna = getNum(p?.deliveryPriceKhulna, p?.delivery_price_khulna, paymentMeta?.deliveryPriceKhulna, dimObj?.deliveryPriceKhulna, local.deliveryPriceKhulna, 150) ?? 150;
  const resolvedDeliveryBarishal = getNum(p?.deliveryPriceBarishal, p?.delivery_price_barishal, paymentMeta?.deliveryPriceBarishal, dimObj?.deliveryPriceBarishal, local.deliveryPriceBarishal, 150) ?? 150;
  const resolvedDeliverySylhet = getNum(p?.deliveryPriceSylhet, p?.delivery_price_sylhet, paymentMeta?.deliveryPriceSylhet, dimObj?.deliveryPriceSylhet, local.deliveryPriceSylhet, 150) ?? 150;
  const resolvedDeliveryRangpur = getNum(p?.deliveryPriceRangpur, p?.delivery_price_rangpur, paymentMeta?.deliveryPriceRangpur, dimObj?.deliveryPriceRangpur, local.deliveryPriceRangpur, 150) ?? 150;
  const resolvedDeliveryMymensingh = getNum(p?.deliveryPriceMymensingh, p?.delivery_price_mymensingh, paymentMeta?.deliveryPriceMymensingh, dimObj?.deliveryPriceMymensingh, local.deliveryPriceMymensingh, 150) ?? 150;

  // Canonical Delivery Charge field: delivery_charge
  const resolvedDeliveryCharge = getNum(
    p?.delivery_charge,
    p?.delivery_charges,
    p?.deliveryCharge,
    local.deliveryCharge,
  ...
  return {
    id,
    code: getStr(p?.code, p?.product_code, paymentMeta.code, dimObj?.code, local.code, `XP-101`),
    ...
    isPinned: getBool(p?.isPinned, p?.is_pinned, paymentMeta.isPinned, dimObj?.isPinned, local.isPinned, false),
    deliveryPrice: resolvedDeliveryPrice,
    deliveryPriceDhaka: resolvedDeliveryDhaka,
    deliveryPriceChattogram: resolvedDeliveryChattogram,
    deliveryPriceRajshahi: resolvedDeliveryRajshahi,
    deliveryPriceKhulna: resolvedDeliveryKhulna,
    deliveryPriceBarishal: resolvedDeliveryBarishal,
    deliveryPriceSylhet: resolvedDeliverySylhet,
    deliveryPriceRangpur: resolvedDeliveryRangpur,
    deliveryPriceMymensingh: resolvedDeliveryMymensingh,
    ...
    timer_end_at: resolvedTimerEndTime,
    timer_enabled: resolvedTimerEnabled,
    timerMessage: resolvedTimerMessage,
    timerActive: resolvedTimerActive,
    timerEnabled: resolvedTimerEnabled,
    bkashNumber: getStr(local.bkashNumber, p?.bkashNumber, p?.bkash_number, paymentMeta.bkashNumber, dimObj?.bkashNumber, ""),
    nagadNumber: getStr(local.nagadNumber, p?.nagadNumber, p?.nagad_number, paymentMeta.nagadNumber, dimObj?.nagadNumber, ""),
    paymentType: (getStr(local.paymentType, p?.paymentType, p?.payment_type, paymentMeta.paymentType, dimObj?.paymentType, "cod") as any),
    paymentPercentage: getNum(local.paymentPercentage, p?.paymentPercentage, p?.payment_percentage, paymentMeta.paymentPercentage, dimObj?.paymentPercentage, 10) ?? 10,
    deliveryCharge: resolvedDeliveryCharge,
    deliveryDays: getStr(local.deliveryDays, p?.deliveryDays, p?.delivery_days, paymentMeta.deliveryDays, dimObj?.deliveryDays, "3-5"),
    freeDelivery: getBool(local.freeDelivery, p?.freeDelivery, p?.free_delivery, paymentMeta.freeDelivery, dimObj?.freeDelivery, false),
...
async function upsertProductToSupabase(productPayload: any) {
  ...
  // Resilient column pruning fallback (only for unmapped schema changes)
  let retries = 0;
...
app.post("/api/products", xoroAdminAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (upsertError) {
        console.error("❌ Database save failed on Supabase:", formatSupabaseError(upsertError));
        db.products = db.products.filter(p => String(p.id) !== String(newProduct.id));
        saveDB();
        return res.status(500).json({ message: "Database save failed: " + formatSupabaseError(upsertError) });
      }
    } catch (upsertEx: any) {
      console.error("❌ Exception during Supabase product save:", upsertEx.message);
      db.products = db.products.filter(p => String(p.id) !== String(newProduct.id));
      saveDB();
      return res.status(500).json({ message: "Database save failed: " + upsertEx.message });
    }
  } catch (err: any) {
    console.error("❌ Database save exception:", err.message);
    db.products = db.products.filter(p => String(p.id) !== String(newProduct.id));
    saveDB();
    return res.status(500).json({ message: "Database save failed: " + err.message });
  }
...
app.put("/api/products/:id", xoroAdminAuthMiddleware, async (req, res) => {
  ...
  if (idx !== -1) {
    ...
    try {
      ...
      const backupProduct = { ...existingProd };
      try {
        let { error: upsertError } = await upsertProductToSupabase(payload);
        if (upsertError) {
          console.error("❌ Database update failed on Supabase:", formatSupabaseError(upsertError));
          db.products[idx] = backupProduct;
          saveDB();
          return res.status(500).json({ message: "Database update failed: " + formatSupabaseError(upsertError) });
        }
      } catch (upsertEx: any) {
        console.error("❌ Exception during Supabase product update:", upsertEx.message);
        db.products[idx] = backupProduct;
        saveDB();
        return res.status(500).json({ message: "Database update failed: " + upsertEx.message });
      }
    } catch (err: any) {
      console.error("❌ Database update exception:", err.message);
      db.products[idx] = existingProd;
      saveDB();
      return res.status(500).json({ message: "Database update failed: " + err.message });
    }
...
<html lang="en">
  <head>
    ...
    })(window,document,'script','dataLayer','GTM-T3JV4BDM');</script>
  ...
  <body>
    ...
    <noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-T3JV4BDM"
